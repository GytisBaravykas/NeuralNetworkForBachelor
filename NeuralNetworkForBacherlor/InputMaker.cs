...
namespace NeuralNetworkForBacherlor
{
    class InputMaker
    {
        public static readonly int WordCount = 360;//180; //360
        public static List<int> sLen = new List<int>();
        public static int lemmaRecognized = 0;
        public static int inputLenght = 0;
        public static List<string> functionW = new List<string>();
        ...
        {
            //int uniqueWords = annos.Where(y => y.Pos != "nežinomas").Select(x => x.Lemma).Count();
            int uniqueWords = annos.Select(x => x.Lemma).Distinct().Count();
            lemmaRecognized += uniqueWords;
            inputLenght += WordCount * 2;
            //inputs.Add(uniqueWords / (double)WordCount * 2);
        ...
        public static Dictionary<string, int> buildFunctionWords(Annotations annos, List<double> inputs)
        {
            ...
            string[] prieveiksm = new string[] { "" };
            string[] ivardziai = new string[] { "aš", "tu", "jis", "tas", "anas", "šitas", "kitas" };
            string[] jungtukai = new string[] { "ir", "bei", "o", "bet", "tačiau", "tik", "ar", "arba", "todėl", "tad", "nei", "nė", "nebent", "tartum", "kad", "tiktai", "jog", "kol", "nes", "kadangi" };
            string[] dalelyt = new string[] { "antai", "argi", "bene", "bent", "gal", "gi", "jau", "juk", "kažin", "ne", "nebe", "net", "pat", "štai", "tegu", "vien", "vos" };
            ...
            for (int i = 0; i < fw.Length; i++)
            {
                ...
                //var temp = annos.Where(x => String.Equals(x.Pos, fw[i])).Select(x => x.Lemma);
                //functionW.AddRange(temp);
